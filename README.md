...
- The drill-floor migration is in its contraction phase: `DrillFloorElevation` and the redundant Rig-level `IsFixedPlatform` property have been removed from the public model. Platform-only depth is represented exclusively by `FixedPlatformProperties.DrillFloorDepth`; `RigType` is the discriminator. Service startup transactionally removes the two obsolete keys from valid persisted Rig JSON and fails closed if any Rig JSON is malformed.
- The completed backup, audit, and three-deployment data migration are documented in [DRILL-FLOOR-DEPTH-MIGRATION.md](DRILL-FLOOR-DEPTH-MIGRATION.md).
