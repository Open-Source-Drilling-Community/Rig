...
public static class RigRestMcpToolRegistrations
{
    public static IServiceCollection AddRigRestMcpTools(this IServiceCollection services)
    {
        ...
        services.AddLegacyMcpTool("rig_create", "Persist a new complete rig master-data configuration and return it with server-owned CreationDate and LastModificationDate. Generate a non-empty rig.MetaInfo.ID first; an existing UUID produces a conflict. RigType is authoritative: PlatformRig permits FixedPlatformProperties and requires a non-empty ClusterID verified live against the Cluster service; every other RigType forbids FixedPlatformProperties and requires ClusterID null. DrillFloorDepth is a Gaussian WGS84 depth in SI metres and defaults to 0.5 m standard uncertainty when omitted. Dependency failure rejects the write without persisting it.", McpToolArgumentHelpers.CreateRigSchema(),
            InvokeRigCreate);
